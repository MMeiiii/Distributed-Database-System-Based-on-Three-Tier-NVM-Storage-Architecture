...
// 各層SSTable大小為
#define MAX_NUM_DRAM_Element 20 // DRAM_SSTable最大可容元素個數
#define MAX_NUM_Hot_Element 20  // Hot_SSTable最大可容元素個數
#define MAX_NUM_Warm_Element 25 // Warm_SSTabble最大可容元素個數
#define MAX_NUM_Cold_Element 30 // Cold_SSTable最大可容元素個數

// 各層SSTable個數(下層為上一層之兩倍)
#define MAX_NUM_DRAM_SSTable 1  // DRAM_Level最大可容SSTable個數
#define MAX_NUM_Hot_SSTable 10  // Hot_Level最大可容SSTable個數
#define MAX_NUM_Warm_SSTable 20 // Warm_Level最大可容SSTable個數
#define MAX_NUM_Cold_SSTable 40 // Cold_Level最大可容SSTable個數
...
#define NVM_Next 22
#define Disk_Seek_time 5140 // 2570 * 8 = 20560
#define Disk_Next 514
...
void OtherCompaction(int level)
{
    ...
    if (level == MAX_L)
    {
        cout << "Amount is exceed the DataBase\n";
    }
...
void Compaction(int level)
{
    ...
    int MAX_Num_Element = Get_MAX_Num_Element(level);
    int MAX_Num_SSTable = Get_MAX_Num_SSTable(level);
    int Flush_Num = MAX_Num_Element * MAX_Num_SSTable / 2;
    ...
    if (Level[level + 1].empty())
    {
        sort(temp_list.begin(), temp_list.end(), cmp_time);
        Delete_Level_Skip_List(level);
        sort(temp_list.begin(), temp_list.begin() + Flush_Num, cmp_key);
        Flush(level + 1, temp_list, 0, Flush_Num);
        sort(temp_list.begin() + Flush_Num, temp_list.begin() + temp_list_num, cmp_key);
        Flush(level, temp_list, Flush_Num, temp_list_num);
        return;
    }
    ...
    if (temp_list_num <= Flush_Num)
    {
        sort(temp_list.begin(), temp_list.end(), cmp_key);
        Flush(level, temp_list, 0, temp_list_num);
    }
    else
    {
        sort(temp_list.begin(), temp_list.end(), cmp_time);
        sort(temp_list.begin(), temp_list.begin() + Flush_Num, cmp_key);
        overlapped_skip_list = Flush_Overlapped(level + 1, temp_list, 0, Flush_Num); // Flush
        sort(temp_list.begin() + Flush_Num, temp_list.begin() + temp_list_num, cmp_key);
        Flush(level, temp_list, Flush_Num, temp_list_num); // Flush
    }
...
void Search_Key(valueType key)
{
    ...
    for (int i = 1; i <= MAX_L; i++)
    {
        ...
        if (i == 1 || i == 2)
        {
            find_time = find_time + NVM_Seek_time + NVM_Next * i;
        }
        else
        {
            find_time = find_time + Disk_Seek_time + Disk_Next * (i - 2);
        }
